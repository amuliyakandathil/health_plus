...
medicine_entry = medicine_data[medicine_data['_id'] == med_id].iloc[0]  # Assuming there's only one matching entry
...
# Filter time_med1_data based on the conditions
filtered_time_med1 = time_med1_data.copy()
filtered_time_med1 = filtered_time_med1[(filtered_time_med1['time'] < end_date) | (filtered_time_med1['time'] > end_date)]

# Prepare data for plotting
plot_data = []

# Iterate over each unique time and check if it exists in filtered_time_med1
for time_val in unique_times:
    # Filter data for the current time
    time_data = filtered_time_med1[filtered_time_med1['time'].dt.time == time_val]
    
    if len(time_data) > 0:
        # Append the time data to plot_data
        plot_data.extend([(date, time_to_fractional_hours(time_val)) for date in time_data['time']])
    else:
        # If no data exists for this time, append a point at (current_date, y-axis for that time)
...
# Plot missed doses as red crosses on the y-axis at time_of_day from code 1
